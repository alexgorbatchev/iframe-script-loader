...
module.exports = function(scripts, callback) {
  ...
  return iframeScript((function() {
    ...
    var doc, name, win, _fn;
    doc = _arg.doc, win = _arg.win;
    _fn = function(name, src) {
      return results[name] = function(done) {
        return scriptLoader(doc, src, function(err) {
          return done(err, win[name]);
        });
      };
    };
    for (name in scripts) {
      _fn(name, scripts[name]);
    }
    return parallel(results, callback);
  });
};


},{"es5-shim":1,"iframe-script":2,"run-parallel":3,"scriptloader":4}]},{},[5])
(5)
});